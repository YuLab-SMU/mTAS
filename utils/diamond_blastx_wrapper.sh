...
while getopts "i:r:R:o:O:-t:h" arg ; do
    case $arg in
    i)
        #echo infile $OPTARG
        inf=$OPTARG
...
function run_blastx() {
	...
	--outfmt 6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen stitle qseqid qtitle \
	--threads $ths \
	-b8 -c1 
    else
